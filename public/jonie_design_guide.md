...
| `--font-body` | `'Inter', system-ui, sans-serif` | replaces `--font-sans` |
| `--font-sans` | `'Inter', system-ui, sans-serif` | unchanged — utility fallback, same as body |
...
```
@import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,500;0,600;0,700;1,400;1,500&family=Inter:wght@300;400;600;700&family=Noto+Serif+SC:wght@400;500;700&display=swap');
```
...
**Inter as body font**: Clean, highly legible UI sans-serif. Neutral and precise — lets Cormorant Garamond carry the editorial weight while Inter handles all body copy and labels cleanly.
...
**Body text**: `font-body` (Inter), weight 400, `--color-ink`.
...
3. Title line — Inter, weight 300, `1.125rem`, color `rgba(248, 245, 238, 0.75)`, letter-spacing `0.12em`, uppercase: "Entrepreneur · Civic Leader · Cebu"
4. Thin gold horizontal rule: `1px solid #B8860B`, width `80px`, margin `24px 0`
5. Lead paragraph — Inter, weight 400, `1.125rem`, color `rgba(248, 245, 238, 0.85)`, max-width `540px`
...
- Below rule: subhead/descriptor in Inter, weight 400, `1rem`, `--color-ink-muted` (`#6B6457`)
...
- Text: `#F8F5EE` (ink-on-dark), Inter, weight 700, `0.08em` letter-spacing, uppercase
- Padding: `12px 32px`
- Border: none
...
| Name in footer | absent | Inter, weight 700, `0.875rem`, uppercase, `letter-spacing: 0.1em`, color `#F8F5EE` |
...
Changes:
- Add Google Fonts `@import` (Cormorant Garamond, Inter, Noto Serif SC) before `@import "tailwindcss"`
